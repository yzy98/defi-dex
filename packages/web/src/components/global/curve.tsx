...
export const Curve: FC<ICurveProps> = (props: ICurveProps) => {
	...
	useEffect(() => {
		...
		if (props.ethReserve && props.tokenReserve) {
			...
			if (props.addingEth) {
				const newEthReserve =
					props.ethReserve + parseFloat(props.addingEth.toString());
				...
				const amountGained =
					Math.round(
						(10000 * (props.addingEth * props.tokenReserve)) / newEthReserve,
					) / 10000;
				ctx.fillStyle = "#000000";
				ctx.fillText(
				...
				const amountGained =
					Math.round(
						(10000 * (props.addingToken * props.ethReserve)) / newTokenReserve,
					) / 10000;
